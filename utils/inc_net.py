...
def get_convnet(args, pretrained=False):
    ...
    # VPT
        ...
        if args["model_name"]=="aper_adapter" or  args["model_name"]=="aper_norm" :
...
class BaseNet(nn.Module):
    def __init__(self, args, pretrained, is_clr = False):
        ...
        self.rpfc = None
        if is_clr:
            self.simclr = nn.Linear(self.feature_dim, 128).to(device='cuda')
    @property
...
class SimpleVitNet(BaseNet):
    def __init__(self, args, pretrained, is_clr=False):
        super().__init__(args, pretrained, is_clr)
    ...
    def update_fc(self, nb_classes, nextperiod_initialization=None):
        ...
        del self.fc
        self.fc = fc
    def update_rpfc(self, nb_classes):
        rpfc = CosineLinear(self.feature_dim, nb_classes).cuda()
        if self.fc is not None:
            nb_output = self.fc.out_features
            weight = copy.deepcopy(self.fc.weight.data)
            rpfc.sigma.data = self.fc.sigma.data
            weight = torch.cat([weight, torch.zeros(nb_classes - nb_output, self.feature_dim).cuda()])
            rpfc.weight = nn.Parameter(weight)
        del self.rpfc
        self.rpfc = rpfc
    def generate_fc(self, in_dim, out_dim):
        fc = CosineLinear(in_dim, out_dim)
    ...
    def forward(self, x, is_clr=False, rpfc=False):
        if is_clr:
            x = self.convnet(x)
            # print(x.shape)
            features = x
            simclr = self.simclr(features)
            out = None
            return features, simclr, out
        if rpfc:
            x = self.convnet(x)
            out = self.rpfc(x)
            return out
